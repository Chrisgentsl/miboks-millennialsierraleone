...
class ProductService {
  ...
  Future<String> exportInventoryToCSV() async {
    try {
      ...
      // Create CSV header
      String csv =
          'Name,SKU,Quantity,Price,Description,Category,Low Stock Threshold\n';

      // Add product data
      for (var product in products) {
        csv +=
            '"${product.name}","${product.sku}",${product.quantity},${product.price},"${product.description}","${product.category}",${product.lowStockThreshold}\n';
      }
      ...
      if (e.toString().contains('permission-denied')) {
        debugPrint(
          'Error exporting inventory: Permission denied. Check your access rights.',
        );
        throw Exception(
          'Permission denied. You do not have sufficient access rights to export inventory.',
        );
      } else {
